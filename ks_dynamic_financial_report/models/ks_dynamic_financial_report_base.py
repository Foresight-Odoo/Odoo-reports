...
class ks_dynamic_financial_base(models.Model):
    ...
    @api.depends('ks_report_menu_id')
    def _compute_total_partner(self):
        total_partners = self.env['account.move'].search([]).mapped('partner_id')
        for rec in self:
            rec.ks_total_partner_ledger = len(total_partners)
    ...
    ks_total_partner_ledger = fields.Integer('Partner ledger total', compute="_compute_total_partner")
    ...
    hide_debit_credit = fields.Boolean('Hide debit credit', default=True)
    ...
    def _ks_compute_account_balance(self, accounts, ks_df_informations, prv_year_dates=False, current_year=False,
        ...
        if accounts:
            ...
            # print("ks_where_clause==========", ks_where_clause)
            # print("params==========", ks_params)
    ...
    def ks_fetch_report_account_lines(self, ks_df_informations, offset=0, limit=None):
        ...
        if self.id == self.env.ref('ks_dynamic_financial_report.ks_df_partner_ledger0').id:
            ...
            return self.ks_partner_process_data(ks_df_informations, offset, limit)
            ...
            for rec in ks_df_informations.get('ks_differ')['ks_intervals']:
                ...
                if self.ks_analytic_account_visibility and self.ks_analytic_filter and self.display_name != 'Executive Summary':
                    if ks_df_informations.get('analytic_accounts', False):
                        ks_analytic_account_ids = [int(acc['id']) for acc in ks_df_informations['analytic_accounts']]
    ...
    # For partner ledger
    ###########################################################################################
    def ks_partner_process_data(self, ks_df_informations, offset, limit):
        '''
        It is the method for showing summary details of each accounts. Just basic details to show up
        Three sections,
        1. Initial Balance
        ...
        if ks_df_informations.get('ks_partner_ids', []):
            ...
            # print("ks_partner_ids======", ks_partner_ids)
            ks_partner_ids = self.env['account.move'].sudo().search([('partner_id', 'in', [partner['id'] for partner in ks_partner_ids])
                                                                     ]).mapped('partner_id')
        else:
            ks_partner_ids = self.ks_build_aging_where_clause(ks_df_informations, offset, limit)[0]
    ...
    #   Age Receivable
    ######################################################################
    def ks_build_aging_where_clause(self, ks_df_informations, offset, limit):
        # print("info=====", ks_df_informations)
        ...
        # print(ks_df_informations.get('ks_partner_ids').mapped('id'))

        if ks_df_informations.get('ks_partner_ids', []):
            partner_ids = self.env['res.partner'].browse([int(partner['id']) for partner in ks_df_informations.get('ks_partner_ids')])
        else:
            partner_ids = self.env['account.move'].sudo().search([('partner_id', '!=', False)], limit=limit, offset=offset).mapped('partner_id')
    ...
    def ks_partner_aging_process_data(self, ks_df_informations, offset, limit):
        ''' Query Start Here
        ['partner_id':
            {'0-30':0.0,
            '30-60':0.0,
        ...
        ks_partner_ids, WHERE = self.ks_build_aging_where_clause(ks_df_informations, offset, limit)
        # print("sefl2====", ks_partner_ids)
        ks_partner_dict = {}
    ...
    def ks_process_aging_data(self, ks_df_informations, offset=0, ks_partner=0, fetch_range=FETCH_RANGE):
        ...
        ks_company_ids = ks_df_informations.get('company_ids')
        ks_partner_ids, WHERE = self.ks_build_aging_where_clause(ks_df_informations, offset=offset, limit=fetch_range)
        if self.id == self.env.ref('ks_dynamic_financial_report.ks_df_receivable0').id:
            ks_type = 'asset_receivable'
        else:
            ks_type = 'liability_payable'
    ...
    def length_query(self, ks_df_informations):
        domain_clause = []
        journal_domain = []
        if ks_df_informations:
            ...
            for journal in ks_df_informations.get('journals', []):
                if not journal['id'] in ('divider', 'group') and journal['selected']:
                    if not journal_domain:
                        journal_domain.append('journal_id', '=', journal['id'])
                    else:
                        journal_domain.append('|')
                        journal_domain.append(('journal_id', '=', journal['id']))
            
            if ks_df_informations.get('ks_reconciled') and not ks_df_informations.get('ks_unreconciled'):
                # domain_clause += ' AND l.amount_residual = 0'
                domain_clause.append('&')
                domain_clause.append(('amount_residual', '=', 0))
            ...
            if ks_df_informations.get('ks_df_report_account_ids', []):
                # WHERE += ' AND a.id IN %s' % str(tuple(ks_df_informations.get('ks_df_report_account_ids')) + tuple([0]))
                # domain_clause.append('&')
                domain_clause.append(('account_id.id', 'in', ks_df_informations.get('ks_df_report_account_ids')))

            if ks_df_informations.get('company_id', False):
                # WHERE += ' AND l.company_id in %s' % str(tuple(ks_df_informations.get('company_ids')) + tuple([0]))
                # domain_clause.append('&')
                domain_clause.append(('company_id', 'in', ks_df_informations.get('company_ids')))
            if ks_df_informations.get('ks_posted_entries') and not ks_df_informations.get('ks_unposted_entries'):
                # WHERE += " AND m.state = 'posted'"
                # domain_clause.append('&')
                domain_clause.append(('move_id.state', '=', 'posted'))
            elif ks_df_informations.get('ks_unposted_entries') and not ks_df_informations.get('ks_posted_entries'):
                WHERE += " AND m.state = 'draft'"
                # domain_clause.append('&')
                domain_clause.append(('move_id.state', '=', 'draft'))
            else:
                # WHERE += " AND m.state IN ('posted', 'draft') "
                # domain_clause.append('&')
                domain_clause.append(('move_id.state', 'in', ['posted', 'draft']))

            if ks_df_informations.get('initial_balance') and self.ks_date_filter.get('ks_process') == 'range':
                # KS_WHERE_FULL = WHERE + " AND l.date <= '%s'" % ks_df_informations['date'].get('ks_end_date')
                domain_clause.append(('date', '<=', str(ks_df_informations['date'].get('ks_end_date'))))
            else:
                if self.ks_date_filter.get('ks_process') == 'range':
                    domain_clause.append(
                        ('date', '>=', str(ks_df_informations['date'].get('ks_start_date'))),
                    )
                    domain_clause.append(
                        ('date', '<=', str(ks_df_informations['date'].get('ks_end_date'))),
                    )
                    # KS_WHERE_FULL = WHERE + " AND l.date >= '%s'" % ks_df_informations['date'].get(
                    #     'ks_start_date') + " AND l.date <= '%s'" % ks_df_informations['date'].get('ks_end_date')
                else:
                    domain_clause.append(('date', '<=', str(ks_df_informations['date'].get('ks_end_date'))))
                    # KS_WHERE_FULL = WHERE + " AND l.date <= '%s'" % ks_df_informations['date'].get('ks_end_date')
        return domain_clause + journal_domain


    def ks_length_info(self, ks_df_informations=False):
        total_partners = self.env['account.move'].search([('state', 'in', ['posted', 'draft']), ('partner_id', '!=', False)]).mapped('partner_id')
        domain = self.length_query(ks_df_informations)
        # print("domain======", domain)
        total_credits = self.env['account.move.line'].search([('partner_id', 'in', total_partners.ids)]+domain).mapped('credit')
        total_debits = self.env['account.move.line'].search([('partner_id', 'in', total_partners.ids)]+domain).mapped('debit')
        total_balances = sum(total_debits) - sum(total_credits)
        # for partner in total_partners:
        #     move_lines_credit = self.env['account.move.line'].search([('partner_id', '=', partner.id)]+domain).mapped('credit')
        #     move_lines_debit = self.env['account.move.line'].search([('partner_id', '=', partner.id)]+domain).mapped('debit')
        #     total_credit += sum(move_lines_credit)
        #     total_debit += sum(move_lines_debit)
        #     total_balance += (sum(move_lines_debit) - sum(move_lines_credit))
            # total_debit += partner.debit
        return {
            'total_partners': len(total_partners),
            'total_credit': sum(total_credits),
            'total_debit': sum(total_debits),
            'total_balance': total_balances,
        }
    def ks_get_dynamic_fin_info(self, ks_df_informations, offset, limit):
        ...
        if self.id == self.env.ref('ks_dynamic_financial_report.ks_df_tb0').id:
            ...
            ks_period_dict, ks_partner_dict = self.ks_partner_aging_process_data(ks_df_informations, offset, limit)
            ...
            ks_report_lines, ks_initial_balance, ks_current_balance, ks_ending_balance = self.ks_fetch_report_account_lines(
                ks_df_informations, offset, limit)
        company_id = self.env.company
    ...
    def ks_get_partner_name(self, ks_df_informations):
        ...
        '''
        # print("self=====", ks_df_informations['ks_partner_ids'])
        ks_selected_partner_ids = [int(partner['id']) for partner in ks_df_informations['ks_partner_ids']]
    ...
    def _ks_construct_partner_filter(self, ks_df_informations, ks_earlier_informations=None):
        ...
        # print("Earlier information===", ks_earlier_informations)
    ...
    def ks_fetch_analytic_accounts(self, ks_df_informations, ks_eariler_informations):
        ...
        analytic_account_ids = [int(acc['id']) for acc in ks_df_informations['analytic_accounts']]
    ...
    def ks_df_show_move_line(self, ks_df_informations, ks_parameter=None):
        ...
        ks_view_id = self.env.ref("%s.%s" % (ks_module, ks_view_name), raise_if_not_found=False)
        return {
            'type': 'ir.actions.act_window',
            'view_mode': 'form',
            'views': [(ks_view_id.id, 'form')],
            'res_model': ks_document,
            'view_id': ks_view_id.id,
            'res_id': ks_target.id,
            'context': ctx,
        }
    ...
    def ks_show_df_journal_items(self, ks_df_informations, ks_parameter=None):
        ...
        if ks_df_informations:
            ...
            if ks_df_informations.get('analytic_accounts'):
                analytic_ids = [int(r['id']) for r in ks_df_informations['analytic_accounts']]
