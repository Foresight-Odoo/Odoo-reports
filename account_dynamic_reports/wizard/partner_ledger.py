...
class InsPartnerLedger(models.TransientModel):
    ...
    balance_less_than_zero = fields.Boolean(string='Payables')
    balance_greater_than_zero = fields.Boolean(string='Receivables')
    type = fields.Selection([('asset_receivable', 'Receivable Only'), ('liability_payable', 'Payable only')], string='Account Type', required=False)
    ...
    product_ids = fields.Many2many(
        'product.product', string='Partners'
    )
    quantity = fields.Integer(string="Quantity")
    partner_count_from = fields.Integer(string="Partner Count From", default=1)
    partner_count_to = fields.Integer(string="Partner Count To", default=3000)
    ...
    def write(self, vals):
        ...
        if vals.get('product_ids'):
            vals.update({'product_ids': vals.get('product_ids')})
        if vals.get('product_ids') == []:
            vals.update({'product_ids': [(5,)]})
    ...
    def validate_data(self):
        ...
        #     if not self.partner_count_from or not self.partner_count_to:
        #         raise ValidationError(_('Please select atleast Partner Count'))
        return True
    ...
    def process_filters(self):
        ...
        if data.get('product_ids', []):
            filters['categories'] = self.env['product.product'].browse(data.get('product_ids', [])).mapped('name')
        else:
            filters['categories'] = ['All']
    ...
    def build_detailed_move_lines(self, offset=0, partner=0, fetch_range=FETCH_RANGE):
        ...
        sql = ('''
                ...
                JOIN account_move m ON (l.move_id=m.id)
                JOIN account_account a ON (l.account_id=a.id)
                LEFT JOIN product_product n ON (l.product_id=n.id)
                --LEFT JOIN account_account_type AS ty ON a.user_type_id = ty.id
                --LEFT JOIN account_analytic_account anl ON (l.analytic_account_id=anl.id)
                LEFT JOIN res_currency c ON (l.currency_id=c.id)
                LEFT JOIN res_currency cc ON (l.company_currency_id=cc.id)
                ...
                JOIN account_journal j ON (l.journal_id=j.id)
                WHERE %s
                GROUP BY l.id, l.partner_id, a.name, l.account_id, l.date, j.code, l.currency_id, l.amount_currency, l.name, n.id, l.quantity, m.id, m.name, c.rounding, cc.id, cc.rounding, cc.position, c.position, c.symbol, cc.symbol, p.name
                ORDER BY %s
                OFFSET %s ROWS
                FETCH FIRST %s ROWS ONLY
    ...
    def process_data(self):
        ...
        if data.get('partner_ids', []):
            ...
            partner_ids = self.env['res.partner'].search(partner_company_domain, offset=self.partner_count_from, limit=self.partner_count_to)
        ...
        for partner in partner_ids:
            ...
            for row in cr.dictfetchall():
                ...
                if (data.get('display_accounts') == 'balance_not_zero' and currency.is_zero(row['debit'] - row['credit'])) \
                    ...
                    move_lines[partner.id]['balance_amount'] += balance_amount
                    move_lines[partner.id]['debit_amount'] += debit_amount
                    move_lines[partner.id]['credit_amount'] += credit_amount
        return move_lines
    ...
    def get_filters(self, default_filters={}):
        ...
        partners = self.partner_ids if self.partner_ids else self.env['res.partner'].search(partner_company_domain, offset=self.partner_count_from, limit=self.partner_count_to)
        categories = self.partner_category_ids if self.partner_category_ids else self.env['res.partner.category'].search([])
        product = self.product_ids if self.product_ids else self.env['product.product'].search([])
        ...
        filter_dict = {
            ...
            'product_list': [(c.id, c.name) for c in product],
    ...
    def action_pdf(self):
        ...
        return self.env.ref('account_dynamic_reports.action_print_partner_ledger').with_context(landscape=True).report_action(
    ...
    def action_xlsx(self):
        ...
        if filter.get('include_details', False):
            ...
            sheet.write(row_pos, 4, _('Entry Label'),
                        format_header)
            sheet.write(row_pos, 5, _('Product Name'),
                        format_header)
            sheet.write(row_pos, 6, _('Quantity'),
                        format_header)
                    ...
                    for sub_line in sub_lines:
                        if sub_line.get('move_name') == 'Initial Balance':
                            ...
                            sheet.write(row_pos, 4, sub_line.get('lname') or '',
                                        line_header_light)
                            sheet.write(row_pos, 5, sub_line.get('product_name') or '',
                                        line_header_light)
                            sheet.write(row_pos, 6, sub_line.get('quantity') or '',
                                        line_header_light)
